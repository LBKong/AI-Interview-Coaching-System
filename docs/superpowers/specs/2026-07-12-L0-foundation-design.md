...
- **凝视代理**：MediaPipe FaceLandmarker 拿人脸关键点 → 推出头部朝向/视线朝向 → 判定"是否在看镜头"。**做到"看镜头 vs 看别处能明显区分"即可**，不追求精确视线追踪。阈值要多调几次。
...
- 前端：**纯 HTML/JS**（免构建）
- 后端：**FastAPI + uvicorn**
- 环境：Python 3.13、Node 26、Homebrew 可用；需安装 ffmpeg（faster-whisper 解码用）。
